...
export default function ModalPublicidad({
  ...
  return (
    ...
    >
      <div className="relative bg-white shadow-2xl w-full max-w-lg md:max-w-4xl grid grid-rows-[auto,minmax(0,1fr)] md:grid-rows-1 md:grid-cols-[minmax(280px,0.9fr)_minmax(0,1.3fr)] overflow-hidden animate-fadeIn max-h-[82vh] md:max-h-[82vh]">
        <button
        ...
        >
          {title && (
            <h3 className="hidden md:block text-lg md:text-2xl font-bold text-gray-900 mb-4 tracking-tight">
              {title}
            </h3>
          )}

          <div className="mb-4 sm:mb-5 border border-gray-100 bg-gradient-to-br from-white via-white to-gray-50/50 p-4 sm:p-5 shadow-[0_15px_30px_rgba(15,23,42,0.06)]">
            <div className="mb-3 flex items-center gap-2.5 flex-wrap">
              <span className="inline-flex items-center px-2.5 py-0.5 text-[9px] font-bold bg-primary text-gray-900 uppercase tracking-widest">
                {promoLabel || copy.launchOffer}
              </span>
              {delivery && (
                <span className="inline-flex items-center px-2.5 py-0.5 text-[9px] font-bold bg-emerald-50 text-emerald-700 border border-emerald-200">
                  {delivery}
                </span>
              )}
            </div>
          ...
          {(description || details) && (
            <div className="mb-3 border border-gray-200 bg-gray-50/80 p-3 sm:p-4 rounded-sm text-xs">
              {description && (
                <p className="text-gray-700 leading-relaxed">{description}</p>
              )}
              {details && <p className="mt-1 text-gray-500 leading-relaxed font-semibold">{details}</p>}
            </div>
          )}

          {Array.isArray(list) && list.length > 0 && (
            <ul className="grid grid-cols-1 md:grid-cols-2 gap-y-1 gap-x-3 mb-3">
              {list.map((item, idx) => (
                <li key={idx} className="flex items-start gap-2">
                  <span className="mt-0.5 inline-flex h-3.5 w-3.5 items-center justify-center rounded-full bg-primary/10 text-primary flex-shrink-0">
                    <svg
                      className="w-2 h-2"
                      viewBox="0 0 20 20"
                      fill="currentColor"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-3">
            <Link
              to="/contacto"
              onClick={onClose}
              className="group relative overflow-hidden bg-secondary text-white px-4 py-2.5 font-bold transition-all duration-300 hover:shadow-xl hover:-translate-y-0.5 text-xs text-center"
            >
            ...
            {whatsappHref && (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="flex items-center justify-center gap-2.5 bg-white border-2 border-gray-900 text-gray-900 px-4 py-2.5 font-bold transition-all duration-300 hover:bg-gray-50 hover:shadow-lg hover:-translate-y-0.5 text-xs"
                onClick={() => {
                  if (window.fbq) {
